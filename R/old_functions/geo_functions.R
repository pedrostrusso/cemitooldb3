...
#' Get GPL ids which contain GDS associated
#'
#' This function is used to get the GPL ids which contain an associated 
#' GDS dataset.
#'
#' @param verbose 
#'
...
#' @examples
#' \dontrun{
#' gpl_ids <- get_gds_gpl_ids()
#' }
#' 
get_gds_gpl_ids <- function(verbose=TRUE){
    ...
    for(seq_start in seq(0, study_count, 50)){
        ...
        search_res <- rentrez::entrez_search(db="gds", term="gds[ETYP]", 
                                             use_history = TRUE,
        ...
        for(id in res_ids){
            ...
            platforms[length(platforms) + 1] <- summary_res$gpl
        }
    }
    platforms <- paste0("GPL", platforms)
    return(platforms)
}
